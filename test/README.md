...
| `products.test.js` | P2 | 商品主檔單一化：migration v12 自癒、雲端整包覆蓋洗不掉、精靈商品庫＝主檔同一物件、基準不可刪只能停用、系列/規格/訂製年數、商品頁管庫存、工業區CRM 讀同一份 |
| `trials.test.js` | T2 | 每次試用一筆：migration v11 打包、進行中／已回收語意（推進表只算未回收）、回收日 X 分級、各次回收待辦、新一次試用與 3 次提醒、回收結果四選一（含報價一週追蹤）、表單本次試用、首頁／長官快照 |
