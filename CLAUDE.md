...
3. **拖把價格待確認**：主系統商品表（NSAC 2W/190、S-20 2W/100…）與合約內建庫 `CM_CATALOG`（NSAC 4W/200、S-20／M-90 無價）不一致，使用者說要先確認。使用者將提供**完整價格資料庫**，屆時整批更新 `DEFAULT_PRODUCTS` 與 `CM_CATALOG`（兩份要對齊，長期應合併成一份）。
...
- **商品價格邏輯**（完整規則見 docs/ARCHITECTURE.md「金額算法」）：`price`＝商品**自身週期**的每次更換單價；定價來源主系統商品庫優先、`CM_CATALOG` 後援；2W 商品選 4W 才 ×2，4W 商品（DOME、芳香）不加倍。地墊 4W 換**不預設打折**，只顯示建議 8 折，折多少業務逐案決定。
