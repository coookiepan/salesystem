...
## 📌 下階段待辦（使用者指定，每次回覆最後要提醒）

1. **客戶狀態選項**：待使用者說明具體需求後動工（改叫法→顯示名稱自訂；新階段→做進狀態骨架，勿做通用編輯器）。
2. **日報產出改進**：方向已議定——自動預填當日素材（拜訪/試用/成約/工地）成草稿＋分段一鍵複製＋LINE 白話版；不自動寫入公司表格。
3. **商品主檔已於 2026-09 單一化**：`DB.products` 是全系統唯一商品來源（`CM_CATALOG` 已刪除），`DEFAULT_PRODUCTS` 是價格表基準且**每個資料進入點都會自癒補回**（見 docs/ARCHITECTURE.md「商品主檔」）。價格變動一律在「商品」頁維護、同步 Sheet；**基準商品不可刪除，只能取消啟用**。
...
- **商品資料唯一來源＝`DB.products`**：模組不得自帶第二份商品清單；新增欄位請加在 `DEFAULT_PRODUCTS` 並由 `ensureProductBaseline()` 補給舊資料。
- **商品價格邏輯**（完整規則見 docs/ARCHITECTURE.md「金額算法」；唯一入口 `unitPriceAt`，精靈經 `HOST.unitAt` 共用）：`price`＝商品**自身週期**的每次更換單價；`price4w`＝2W 商品表定的 4 週換單價（拖把：NSAC 200、S-20 160…；SHH 兩週四週同價 140）；沒有 `price4w` 的 2W 商品選 4W → ×2（地墊 4W 原價）；4W 商品（DOME、芳香）不看所選週期。定價來源主系統商品庫優先、`CM_CATALOG` 後援。**不預設打折**：建議檔（地墊 4W 三檔 9／85／8 折，其他 9／85 折）以可點的 `.tier` 一鍵帶入，折多少業務逐案決定。
